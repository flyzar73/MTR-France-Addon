...
function drawMainContent(ctx, pids, arrival) {
    const eta = (arrival.departureTime() - Date.now()) / 60000;

    // Check if departure is in progress
    if (eta <= 0.15) {
        drawDepartureInProgress(ctx, pids);
        return;
    }
    ...
    drawDepartureTime(ctx, pids, arrival);
}
...
function drawDepartureInProgress(ctx, pids) {
    Texture.create("Departure In Progress")
        .texture("jsblock:idf/images/backgrounds/depart_en_cours.png")
        .size(pids.width, pids.height)
        .draw(ctx);
}
...
function drawDepartureTime(ctx, pids, arrival) {
    const eta = (arrival.departureTime() - Date.now()) / 60000;
    let etaText;
