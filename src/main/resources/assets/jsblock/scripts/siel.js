...
function render(ctx, state, pids) {
    ...
    if (firstArrival != null) {
        ...
        let eta1 = Math.round(((firstArrival.arrivalTime() - Date.now()) + 20000) / 60000);
        if (eta1 > 99) {
            Text.create("Arrival 1").text("++").color(0xECBB00).pos(47, 35).scale(4.5).rightAlign().draw(ctx);
        } else if (eta1 <= 0) {
            Text.create("Arrival 1").text("0").color(0xECBB00).pos(47, 35).scale(4.5).rightAlign().draw(ctx);
        } else {
    ...
    if (secondArrival != null) {
        let eta2 = Math.round(((secondArrival.arrivalTime() - Date.now()) + 20000) / 60000);
        if (eta2 > 99) {
