...
function render(ctx, state, pids) {
    ...
    if (arrival != null) {
        ...
        let eta = (arrival.departureTime() - Date.now()) / 60000;
        if (eta <= 0.15) {
            Texture.create("Background").texture("jsblock:textures/backgrounds/depart_en_cours.png").size(pids.width, pids.height).draw(ctx);
        } else if (eta <= 0.33) {
            Text.create("ETA Text").text("imminent").color(0xFFFFFF).size(30, 7).scaleXY().rightAlign().pos(pids.width - 2, 20).draw(ctx);
        } else {
            Text.create("ETA Text").text(Math.ceil(eta) + " min").color(0xFFFFFF).size(30, 7).scaleXY().rightAlign().pos(pids.width - 2, 20).draw(ctx);
        }
    } else {
